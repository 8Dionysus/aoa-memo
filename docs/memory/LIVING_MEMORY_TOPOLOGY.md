...
| `aoa-sdk` | return hints, dispatch entrypoints, route compression pressure | route-only unless a port is added | `aoa-memo` for route-memory evidence only | router, SDK, agents | dispatch behavior and navigation policy |
