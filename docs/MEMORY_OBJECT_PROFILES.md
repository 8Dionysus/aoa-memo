...
- `decision` -> `schemas/decision.schema.json` -> `mechanics/checkpoint/parts/approval-and-health-records/examples/checkpoint_approval_record.example.json`
- `pattern` -> `schemas/pattern.schema.json` -> `examples/pattern.example.json`
- `bridge` -> `schemas/bridge.schema.json` -> `mechanics/consumer-handoff/parts/kag-tos-bridge-handoff/examples/bridge.kag-lift.example.json`
