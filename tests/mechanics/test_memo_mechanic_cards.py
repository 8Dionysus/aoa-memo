...
def test_memo_mechanic_cards_are_current_and_valid() -> None:
    run_script("scripts/mechanics/build_memo_mechanic_cards.py", "--check")
    run_script("scripts/mechanics/validate_memo_mechanic_cards.py")
    payload = json.loads((REPO_ROOT / "generated" / "mechanics" / "memo_mechanic_cards.min.json").read_text())
