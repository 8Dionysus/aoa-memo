...
def test_memo_mechanic_readiness_is_current_and_valid() -> None:
    run_script("scripts/mechanics/build_memo_mechanic_readiness.py", "--check")
    run_script("scripts/mechanics/validate_memo_mechanic_readiness.py")
    payload = json.loads((REPO_ROOT / "generated" / "mechanics" / "memo_mechanic_readiness.min.json").read_text())
...
def test_memo_mechanic_readiness_covers_all_packages() -> None:
    ...
    assert payload["mechanic_index_ref"] == "generated/mechanics/memo_mechanics.min.json"
    assert payload["artifact_inventory_ref"] == "generated/mechanics/mechanic_artifacts.min.json"
