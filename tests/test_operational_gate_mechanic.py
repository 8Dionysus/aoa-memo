...
class OperationalGateMechanicTestCase(unittest.TestCase):
    ...
    def test_operational_gate_keeps_root_technical_contracts_root_owned(self) -> None:
    ...
    def test_deployment_gate_schema_requires_admission_fields(self) -> None:
        schema = json.loads(
            (REPO_ROOT / "schemas" / "deployment_incident_memory_gate_v1.json").read_text()
        )
        example = json.loads(
            (REPO_ROOT / "examples" / "deployment_incident_memory_gate.example.json").read_text()
        )
