...
def test_memo_mechanic_landing_logs_are_current_and_valid() -> None:
    run_script("scripts/build_memo_mechanic_landing_logs.py", "--check")
    run_script("scripts/validate_memo_mechanic_landing_logs.py")
    payload = json.loads((REPO_ROOT / "generated" / "memo_mechanic_landing_logs.min.json").read_text())
...
def test_memo_mechanic_landing_logs_cover_every_package_receipt() -> None:
    ...
    for slug, package in packages.items():
        ...
        assert "python scripts/release_check.py" in package["validation_refs"]
