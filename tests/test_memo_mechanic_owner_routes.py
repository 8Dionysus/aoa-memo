...
def test_memo_mechanic_owner_routes_are_current_and_valid() -> None:
    run_script("scripts/build_memo_mechanic_owner_routes.py", "--check")
    run_script("scripts/validate_memo_mechanic_owner_routes.py")
    payload = json.loads((REPO_ROOT / "generated" / "memo_mechanic_owner_routes.min.json").read_text())
