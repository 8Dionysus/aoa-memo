...
MEMORY_GENERATED = GENERATED / "memory"
EXAMPLES = ROOT / "examples"
RECALL_EXAMPLES = EXAMPLES / "recall"
...
def validate_router_recall_contract(path: Path, expected_mode: str, expected_capsule_surface: str) -> None:
    ...
    if data.get("inspect_surface") != "generated/memory/memory_catalog.min.json":
        errors.append("inspect_surface must stay generated/memory/memory_catalog.min.json")
    if data.get("capsule_surface") != expected_capsule_surface:
        errors.append(f"capsule_surface must stay {expected_capsule_surface}")
    if data.get("expand_surface") != "generated/memory/memory_sections.full.json":
        errors.append("expand_surface must stay generated/memory/memory_sections.full.json")
...
def validate_surface_alignment() -> None:
    catalog = load_json(MEMORY_GENERATED / "memory_catalog.json")
    catalog_min = load_json(MEMORY_GENERATED / "memory_catalog.min.json")
    capsules = load_json(MEMORY_GENERATED / "memory_capsules.json")
    sections = load_json(MEMORY_GENERATED / "memory_sections.full.json")
...
def main() -> int:
    validate_catalog(MEMORY_GENERATED / "memory_catalog.json", require_relations=True)
    validate_catalog(MEMORY_GENERATED / "memory_catalog.min.json", require_relations=False)
    validate_capsules(MEMORY_GENERATED / "memory_capsules.json")
    validate_sections(MEMORY_GENERATED / "memory_sections.full.json")
    validate_router_recall_contract(
        RECALL_EXAMPLES / "recall_contract.router.semantic.json",
        "semantic",
        "generated/memory/memory_capsules.json",
    )
    validate_router_recall_contract(
        RECALL_EXAMPLES / "recall_contract.router.lineage.json",
        "lineage",
        "generated/memory/memory_capsules.json",
    )
