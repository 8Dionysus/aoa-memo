...
LIFECYCLE_EXAMPLES = EXAMPLES / "lifecycle"
MEMORY_OBJECT_EXAMPLES = EXAMPLES / "memory-objects"
...
def main() -> int:
    ...
    superseded_claim = validate(
        LIFECYCLE_EXAMPLES / "claim.superseded.example.json",
        memory_validator,
        memory_object_ref_errors,
    )
    anchor = validate(
        MEMORY_OBJECT_EXAMPLES / "anchor.example.json",
        memory_validator,
        memory_object_ref_errors,
    )
    replacement_claim = validate(
        LIFECYCLE_EXAMPLES / "claim.current-entrypoint.example.json",
        memory_validator,
        memory_object_ref_errors,
    )
    retracted_claim = validate(
        LIFECYCLE_EXAMPLES / "claim.retracted.example.json",
        memory_validator,
        memory_object_ref_errors,
    )
    supersession_event = validate(
        LIFECYCLE_EXAMPLES / "audit_event.supersession.example.json",
        memory_validator,
        memory_object_ref_errors,
    )
    retraction_event = validate(
        LIFECYCLE_EXAMPLES / "audit_event.retraction.example.json",
        memory_validator,
        memory_object_ref_errors,
    )
    lifecycle_thread = validate(LIFECYCLE_EXAMPLES / "provenance_thread.lifecycle.example.json", thread_validator)
