...
DEFAULT_MANIFEST = REPO_ROOT / "docs" / "memory" / "artifact-bundles" / "memory_object_readmodels.bundle.json"
DEFAULT_SUBJECT = REPO_ROOT / "generated" / "memory-objects" / "memory_object_catalog.min.json"
DEFAULT_BUNDLE_DIR = REPO_ROOT / "dist" / "abyss-artifact-bundle" / "aoa-memo-memory-object-readmodels"
DEFAULT_REGISTRY_DIR = REPO_ROOT / "dist" / "abyss-artifact-registry" / "aoa-memo-memory-object-readmodels"
DEFAULT_SUBJECT_STORE_ROOT = REPO_ROOT / "dist" / "abyss-artifact-subjects" / "aoa-memo-memory-object-readmodels"
ARTIFACT_CLASS = "derived_memory_object_readmodel_family"
CONSUMER_INTENT = "agent"
CONSUMER_REF = "aoa-memo:memory-object-readmodels"
SOURCE_REPO = "aoa-memo"
TRUST_ROOT_MODE = "host_managed"
PRODUCER = "aoa-memo memory object surface generator from reviewed corpus and curated examples"
...
def _assert_public_safe_subjects(manifest: Path, subject: Path) -> None:
    ...
    if leaks:
        raise ValueError("memory-object readmodel bundle subjects contain private or machine-local markers: " + ", ".join(leaks))
...
def _assert_expected_controls(artifact_bundles: Any, bundle_dir: Path, verify: dict[str, Any], identity: dict[str, Any]) -> None:
    ...
    if (bundle_dir / artifact_bundles.SBOM_CYCLONEDX_SIDECAR).exists():
        raise ValueError("memory-object readmodel bundle must not emit SBOM before package/release-bundle status")
...
def _verify_private_readmodel_marker(manifest: Path, subject: Path, tmp_root: Path) -> dict[str, Any]:
    candidate = tmp_root / "private-memory-object-catalog.min.json"
    ...
    try:
        ...
        return {
            "ok": "memory-object readmodel bundle subjects contain private or machine-local markers" in str(exc),
            "error": str(exc),
        }
    return {"ok": False, "error": "private memory-object readmodel marker was not detected"}
...
def _verify_terminal_registry_state(
    ...
    revoked = artifact_bundles.write_bundle_registry_record(
        ...
        revocation_reason="aoa-memo memory-object readmodel terminal-state rehearsal",
        source_repo=SOURCE_REPO,
        source_ref=_portable_ref(manifest),
        producer=PRODUCER,
        trust_root_mode=TRUST_ROOT_MODE,
...
def _run_adversarial_checks(
    ...
    with tempfile.TemporaryDirectory(prefix="aoa-memo-memory-object-bundle-negative-", dir=_default_tmp_root()) as tmp:
        tmp_root = Path(tmp)
        checks = {
...
def _validate_in_bundle_dir(
    ...
    *,
    ...
    producer_command = "python scripts/memory/validate_abyss_machine_memory_object_bundle.py"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate aoa-memo memory-object readmodels through OS Abyss artifact bundles."
    )
    ...
    if args.json:
        ...
        print(
            "[ok] abyss-machine aoa-memo memory-object readmodel artifact bundle verified: "
