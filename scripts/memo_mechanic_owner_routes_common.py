...
GENERATED_PATH = REPO_ROOT / "generated" / "memo_mechanic_owner_routes.min.json"
...
GENERATED_BY = "scripts/build_memo_mechanic_owner_routes.py"
...
def validate_payload(payload: dict[str, Any]) -> list[str]:
    ...
    if payload != expected:
        issues.append("generated/memo_mechanic_owner_routes.min.json is stale; run scripts/build_memo_mechanic_owner_routes.py")
    if payload.get("schema_version") != SCHEMA_VERSION:
        issues.append(f"generated/memo_mechanic_owner_routes.min.json must keep schema_version {SCHEMA_VERSION}")
    if payload.get("source_of_truth") != SOURCE_OF_TRUTH:
        issues.append("generated/memo_mechanic_owner_routes.min.json must keep OWNER_MAP.md and README cards as source_of_truth")
    if payload.get("config_ref") != CONFIG_REF:
        issues.append(f"generated/memo_mechanic_owner_routes.min.json must route config_ref to {CONFIG_REF}")
    if payload.get("card_index_ref") != CARD_INDEX_REF:
        issues.append(f"generated/memo_mechanic_owner_routes.min.json must route card_index_ref to {CARD_INDEX_REF}")
    if payload.get("generated_by") != GENERATED_BY:
        issues.append(f"generated/memo_mechanic_owner_routes.min.json must name {GENERATED_BY}")

    packages = payload.get("packages")
    if not isinstance(packages, list):
        issues.append("generated/memo_mechanic_owner_routes.min.json packages must be a list")
        return issues

    for package in packages:
        if not isinstance(package, dict):
            issues.append("generated/memo_mechanic_owner_routes.min.json package entries must be objects")
            continue
    ...
    if not isinstance(owners, dict) or not owners:
        issues.append("generated/memo_mechanic_owner_routes.min.json owners index must be a non-empty object")
    counts = payload.get("counts")
    if isinstance(counts, dict):
        if counts.get("packages") != len(packages):
            issues.append("generated/memo_mechanic_owner_routes.min.json package count mismatch")
        if int(counts.get("owners", 0)) < 8:
            issues.append("generated/memo_mechanic_owner_routes.min.json must expose broad stronger-owner coverage")
        if int(counts.get("stronger_owner_entries", 0)) < int(counts.get("packages", 0)) * 3:
            issues.append("generated/memo_mechanic_owner_routes.min.json must keep at least three stronger-owner entries per package on average")
    ...
    for stop_line in ("not owner acceptance", "proof", "runtime authority", "route dispatch", "role authority"):
        if stop_line not in note:
            issues.append(f"generated/memo_mechanic_owner_routes.min.json generated_note must mention {stop_line!r}")
    return issues
