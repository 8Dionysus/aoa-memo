...
def main() -> int:
    ...
    if args.check:
        ...
        if current != rendered:
            print(
                "[error] generated/mechanic_artifacts.min.json is out of date; "
                "run scripts/build_mechanic_artifact_inventory.py",
                file=sys.stderr,
            )
            return 1
        print("[ok] generated/mechanic_artifacts.min.json is current")
        return 0

    GENERATED_PATH.write_text(rendered, encoding="utf-8")
    print("[ok] wrote generated/mechanic_artifacts.min.json")
    return 0
