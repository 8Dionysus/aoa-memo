...
def main() -> int:
    ...
    if args.check:
        ...
        if current != rendered:
            print(
                "[error] generated/memo_mechanic_readiness.min.json is out of date; "
                "run scripts/build_memo_mechanic_readiness.py",
                file=sys.stderr,
            )
            return 1
        print("[ok] generated/memo_mechanic_readiness.min.json is current")
        return 0

    GENERATED_PATH.write_text(rendered, encoding="utf-8")
    print("[ok] wrote generated/memo_mechanic_readiness.min.json")
    return 0
