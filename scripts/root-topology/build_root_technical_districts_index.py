...
def main() -> int:
    ...
    if args.check:
        ...
        if current != rendered:
            print(
                "[error] generated/root-topology/root_technical_districts.min.json is out of date; "
                "run scripts/root-topology/build_root_technical_districts_index.py",
                file=sys.stderr,
            )
            return 1
        print("[ok] generated/root-topology/root_technical_districts.min.json is current")
        return 0

    GENERATED_PATH.write_text(rendered, encoding="utf-8")
    print("[ok] wrote generated/root-topology/root_technical_districts.min.json")
    return 0
