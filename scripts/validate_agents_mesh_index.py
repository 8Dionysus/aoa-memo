...
def validate(repo_root: Path) -> list[str]:
    ...
    if not isinstance(cards, list) or not cards:
        ...
        for card in cards:
            ...
            seen_paths.add(path)
            if card.get("shape_status") != "canonical":
                issues.append(f"{path}: shape_status must be canonical")
            if card.get("missing_required_snippets"):
                issues.append(f"{path}: generated index records missing snippets")
