...
def main() -> int:
    ...
    if args.check:
        ...
        if current != rendered:
            print(
                "[error] generated/mechanics/memo_mechanics.min.json is out of date; "
                "run scripts/mechanics/build_memo_mechanics_index.py",
                file=sys.stderr,
            )
            return 1
        print("[ok] generated/mechanics/memo_mechanics.min.json is current")
        return 0

    GENERATED_PATH.write_text(rendered, encoding="utf-8")
    print("[ok] wrote generated/mechanics/memo_mechanics.min.json")
    return 0
