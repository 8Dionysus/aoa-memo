...
GENERATED_PATH = REPO_ROOT / "generated" / "mechanics" / "memo_mechanic_cards.min.json"
...
GENERATED_BY = "scripts/mechanics/build_memo_mechanic_cards.py"
...
def validate_payload(payload: dict[str, Any]) -> list[str]:
    ...
    if payload != expected:
        issues.append("generated/mechanics/memo_mechanic_cards.min.json is stale; run scripts/mechanics/build_memo_mechanic_cards.py")
    if payload.get("schema_version") != SCHEMA_VERSION:
        issues.append(f"generated/mechanics/memo_mechanic_cards.min.json must keep schema_version {SCHEMA_VERSION}")
    if payload.get("source_of_truth") != SOURCE_OF_TRUTH:
        issues.append("generated/mechanics/memo_mechanic_cards.min.json must keep mechanic README cards as source_of_truth")
    if payload.get("config_ref") != CONFIG_REF:
        issues.append(f"generated/mechanics/memo_mechanic_cards.min.json must route config_ref to {CONFIG_REF}")
    if payload.get("generated_by") != GENERATED_BY:
        issues.append(f"generated/mechanics/memo_mechanic_cards.min.json must name {GENERATED_BY}")

    packages = payload.get("packages")
    if not isinstance(packages, list):
        issues.append("generated/mechanics/memo_mechanic_cards.min.json packages must be a list")
        return issues
    for package in packages:
        if not isinstance(package, dict):
            issues.append("generated/mechanics/memo_mechanic_cards.min.json package entries must be objects")
            continue
    ...
    if isinstance(counts, dict):
        if counts.get("landed_packages") != counts.get("packages"):
            issues.append("generated/mechanics/memo_mechanic_cards.min.json landed_packages must equal packages")
        if int(counts.get("must_not_claims", 0)) < int(counts.get("packages", 0)) * 3:
            issues.append("generated/mechanics/memo_mechanic_cards.min.json must keep at least three stop-lines per package")
