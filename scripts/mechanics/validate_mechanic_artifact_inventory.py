...
def validate_payload(payload: dict[str, Any]) -> list[str]:
    ...
    if payload.get("generated_by") != "scripts/mechanics/build_mechanic_artifact_inventory.py":
    ...
    if payload != expected:
        issues.append(
            f"{GENERATED_PATH.relative_to(REPO_ROOT)} is stale; run scripts/mechanics/build_mechanic_artifact_inventory.py"
        )
...
def validate() -> list[str]:
    if not GENERATED_PATH.exists():
        return [f"{GENERATED_PATH.relative_to(REPO_ROOT)} is missing; run scripts/mechanics/build_mechanic_artifact_inventory.py"]
    try:
