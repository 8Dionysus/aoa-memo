...
def validate_generated_family_contracts(
    ...
    if not isinstance(families, list):
        return ["config/root-topology/root_technical_districts.json: generated_families must be a list"]
    ...
    for index, family in enumerate(families):
        ...
        if not isinstance(family_id, str) or not family_id:
            ...
            issues.append(f"config/root-topology/root_technical_districts.json: duplicate generated family id {family_id}")
        seen_family_ids.add(family_id)

        source_kind = family.get("source_kind")
        if source_kind not in GENERATED_SOURCE_KINDS:
            issues.append(
                f"config/root-topology/root_technical_districts.json: {family_id}.source_kind must be one of "
        ...
        if source_refs == []:
            issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.source_refs must not be empty")
        if validators == []:
            issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.validators must not be empty")
        if source_kind in BUILDER_REQUIRED_GENERATED_SOURCE_KINDS and builders == []:
            issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.builders must not be empty")

        if outputs is None:
            continue
        if outputs == []:
            issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.outputs must not be empty")
        for output in outputs:
            output_path = Path(output)
            if output_path.parts[:1] != ("generated",):
                issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.outputs contains non-root generated path {output}")
            if output_path.name == "AGENTS.md":
                issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.outputs must not list route cards")
            if output in output_to_family:
                issues.append(
                    f"config/root-topology/root_technical_districts.json: generated output {output} appears in both "
    ...
    for missing in sorted(allowed_generated_files - covered_outputs):
        issues.append(f"config/root-topology/root_technical_districts.json: generated output {missing} lacks a generated_families contract")
    for extra in sorted(covered_outputs - allowed_generated_files):
        issues.append(f"config/root-topology/root_technical_districts.json: generated_families covers non-allowed generated output {extra}")
...
def validate_script_family_contracts(
    ...
    if not isinstance(families, list):
        return ["config/root-topology/root_technical_districts.json: script_families must be a list"]
    ...
    for index, family in enumerate(families):
        ...
        if not isinstance(family_id, str) or not family_id:
            ...
            issues.append(f"config/root-topology/root_technical_districts.json: duplicate script family id {family_id}")
        seen_family_ids.add(family_id)

        role = family.get("role")
        if role not in SCRIPT_FAMILY_ROLES:
            issues.append(
                f"config/root-topology/root_technical_districts.json: {family_id}.role must be one of "
        ...
        if scripts == []:
            issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.scripts must not be empty")
        if covered_by == []:
            issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.covered_by must not be empty")
        ...
        for script in scripts:
            ...
            if script_path.parts[:1] != ("scripts",):
                issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.scripts contains non-root script path {script}")
            if script_path.name == "AGENTS.md":
                issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.scripts must not list route cards")
            issues.extend(validate_local_ref(script, f"{family_id}.scripts"))
            if script in script_to_family:
                issues.append(
                    f"config/root-topology/root_technical_districts.json: root script {script} appears in both "
    ...
    for missing in sorted(allowed_script_files - covered_scripts):
        issues.append(f"config/root-topology/root_technical_districts.json: root script {missing} lacks a script_families contract")
    for extra in sorted(covered_scripts - allowed_script_files):
        issues.append(f"config/root-topology/root_technical_districts.json: script_families covers non-allowed root script {extra}")
...
def validate_test_family_contracts(
    ...
    if not isinstance(families, list):
        return ["config/root-topology/root_technical_districts.json: test_families must be a list"]
    ...
    for index, family in enumerate(families):
        ...
        if not isinstance(family_id, str) or not family_id:
            ...
            issues.append(f"config/root-topology/root_technical_districts.json: duplicate test family id {family_id}")
        seen_family_ids.add(family_id)
        ...
        if tests == []:
            issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.tests must not be empty")
        if protects == []:
            issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.protects must not be empty")
        ...
        for test in tests:
            ...
            if test_path.parts[:1] != ("tests",):
                issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.tests contains non-root tests path {test}")
            if test_path.name == "AGENTS.md":
                issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.tests must not list route cards")
            issues.extend(validate_local_ref(test, f"{family_id}.tests"))
            if test in test_to_family:
                issues.append(
                    f"config/root-topology/root_technical_districts.json: root test {test} appears in both "
    ...
    for missing in sorted(allowed_test_files - covered_tests):
        issues.append(f"config/root-topology/root_technical_districts.json: root test {missing} lacks a test_families contract")
    for extra in sorted(covered_tests - allowed_test_files):
        issues.append(f"config/root-topology/root_technical_districts.json: test_families covers non-allowed root test {extra}")
...
def validate_schema_family_contracts(
    ...
    if not isinstance(families, list):
        return ["config/root-topology/root_technical_districts.json: schema_families must be a list"]
    ...
    for index, family in enumerate(families):
        ...
        if not isinstance(family_id, str) or not family_id:
            ...
            issues.append(f"config/root-topology/root_technical_districts.json: duplicate schema family id {family_id}")
        seen_family_ids.add(family_id)
        ...
        if schemas == []:
            issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.schemas must not be empty")
        if source_refs == []:
            issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.source_refs must not be empty")
        if validators == []:
            issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.validators must not be empty")
        ...
        for schema in schemas:
            ...
            if schema_path.parts[:1] != ("schemas",):
                issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.schemas contains non-root schema path {schema}")
            if schema_path.name == "AGENTS.md":
                issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.schemas must not list route cards")
            issues.extend(validate_local_ref(schema, f"{family_id}.schemas"))
            if schema in schema_to_family:
                issues.append(
                    f"config/root-topology/root_technical_districts.json: root schema {schema} appears in both "
    ...
    for missing in sorted(allowed_schema_files - covered_schemas):
        issues.append(f"config/root-topology/root_technical_districts.json: root schema {missing} lacks a schema_families contract")
    for extra in sorted(covered_schemas - allowed_schema_files):
        issues.append(f"config/root-topology/root_technical_districts.json: schema_families covers non-allowed root schema {extra}")
...
def validate_example_family_contracts(
    ...
    if not isinstance(families, list):
        return ["config/root-topology/root_technical_districts.json: example_families must be a list"]
    ...
    for index, family in enumerate(families):
        ...
        if not isinstance(family_id, str) or not family_id:
            ...
            issues.append(f"config/root-topology/root_technical_districts.json: duplicate example family id {family_id}")
        seen_family_ids.add(family_id)
        ...
        if examples == []:
            issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.examples must not be empty")
        if source_refs == []:
            issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.source_refs must not be empty")
        if validators == []:
            issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.validators must not be empty")
        ...
        for example in examples:
            ...
            if example_path.parts[:1] != ("examples",):
                issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.examples contains non-root example path {example}")
            if example_path.name == "AGENTS.md":
                issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.examples must not list route cards")
            issues.extend(validate_local_ref(example, f"{family_id}.examples"))
            if example in example_to_family:
                issues.append(
                    f"config/root-topology/root_technical_districts.json: root example {example} appears in both "
    ...
    for missing in sorted(allowed_example_files - covered_examples):
        issues.append(f"config/root-topology/root_technical_districts.json: root example {missing} lacks an example_families contract")
    for extra in sorted(covered_examples - allowed_example_files):
        issues.append(f"config/root-topology/root_technical_districts.json: example_families covers non-allowed root example {extra}")
...
def validate_config_family_contracts(
    ...
    if not isinstance(families, list):
        return ["config/root-topology/root_technical_districts.json: config_families must be a list"]
    ...
    for index, family in enumerate(families):
        ...
        if not isinstance(family_id, str) or not family_id:
            ...
            issues.append(f"config/root-topology/root_technical_districts.json: duplicate config family id {family_id}")
        seen_family_ids.add(family_id)
        ...
        if configs == []:
            issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.configs must not be empty")
        if source_refs == []:
            issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.source_refs must not be empty")
        if validators == []:
            issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.validators must not be empty")
        ...
        for config_path_text in configs:
            ...
            if config_path.parts[:1] != ("config",):
                issues.append(
                    f"config/root-topology/root_technical_districts.json: {family_id}.configs contains non-root config path {config_path_text}"
                )
            if config_path.name == "AGENTS.md":
                issues.append(f"config/root-topology/root_technical_districts.json: {family_id}.configs must not list route cards")
            issues.extend(validate_local_ref(config_path_text, f"{family_id}.configs"))
            if config_path_text in config_to_family:
                issues.append(
                    f"config/root-topology/root_technical_districts.json: root config {config_path_text} appears in both "
    ...
    for missing in sorted(allowed_config_files - covered_configs):
        issues.append(f"config/root-topology/root_technical_districts.json: root config {missing} lacks a config_families contract")
    for extra in sorted(covered_configs - allowed_config_files):
        issues.append(f"config/root-topology/root_technical_districts.json: config_families covers non-allowed root config {extra}")
...
def validate_manifest_policy_contract(
    ...
    if source_refs == []:
        issues.append("config/root-topology/root_technical_districts.json: manifest_policy.source_refs must not be empty")
    if validators == []:
        issues.append("config/root-topology/root_technical_districts.json: manifest_policy.validators must not be empty")
