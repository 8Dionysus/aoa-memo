...
def validate_payload(payload: dict[str, Any]) -> list[str]:
    ...
    if payload != expected:
        issues.append(f"{_rel(GENERATED_PATH)} is stale; run scripts/build_root_technical_districts_index.py")
...
def validate() -> list[str]:
    if not GENERATED_PATH.exists():
        return [f"{_rel(GENERATED_PATH)} is missing; run scripts/build_root_technical_districts_index.py"]
    try:
