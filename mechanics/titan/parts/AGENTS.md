...
`mechanics/titan/parts/` and every active part below it.
...
The active `titan` part rows are selected by `PARTS.md`; each row is
...
The `titan` part route narrows the package operation; it cannot widen memo
...
Tie each `titan` part to one row in `mechanics/titan/PARTS.md`; keep its
...
Use the nearest `VALIDATION.md` route for the affected `titan` part after its
