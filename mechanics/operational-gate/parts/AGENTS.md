...
`mechanics/operational-gate/parts/` and every active part below it.
...
The active `operational-gate` part rows are selected by `PARTS.md`; each row is
...
The `operational-gate` part route narrows the package operation; it cannot widen memo
...
Tie each `operational-gate` part to one row in `mechanics/operational-gate/PARTS.md`; keep its
...
Use the nearest `VALIDATION.md` route for the affected `operational-gate` part after its
