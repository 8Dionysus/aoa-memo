...
WRITEBACK_SCRIPTS_ROOT = REPO_ROOT / "mechanics" / "writeback" / "scripts"
if str(WRITEBACK_SCRIPTS_ROOT) not in sys.path:
    sys.path.insert(0, str(WRITEBACK_SCRIPTS_ROOT))
...
def test_growth_refinery_writeback_lanes_surface_stays_generator_backed() -> None:
    current = load_json("mechanics/writeback/generated/growth_refinery_writeback_lanes.min.json")
