...
OUTPUT_PATH = PART_ROOT / "generated" / "runtime_writeback_targets.min.json"
...
def build_runtime_writeback_targets_payload() -> dict[str, object]:
    ...
    return {
        ...
        "source_of_truth": "mechanics/checkpoint/parts/checkpoint-to-memory-mapping/examples/checkpoint_to_memory_contract.example.json",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        ...
        if current != payload:
            raise SystemExit(
                "[error] mechanics/writeback/parts/runtime-and-temperature/generated/runtime_writeback_targets.min.json is out of date; "
                "run mechanics/writeback/parts/runtime-and-temperature/scripts/generate_runtime_writeback_targets.py"
            )
        print("[ok] mechanics/writeback/parts/runtime-and-temperature/generated/runtime_writeback_targets.min.json is current")
        return 0

    write_output(payload)
    print("[ok] wrote mechanics/writeback/parts/runtime-and-temperature/generated/runtime_writeback_targets.min.json")
    return 0
