...
TARGETS_PATH = PART_ROOT / "generated" / "runtime_writeback_targets.min.json"
CONTRACT_PATH = (
    REPO_ROOT
    / "mechanics"
    / "checkpoint"
...
OUTPUT_PATH = PART_ROOT / "generated" / "runtime_writeback_intake.min.json"
...
def build_runtime_writeback_intake_payload() -> dict[str, object]:
    ...
    if not isinstance(targets_payload, dict):
        raise SystemExit("[error] mechanics/writeback/parts/runtime-and-temperature/generated/runtime_writeback_targets.min.json must contain an object")
    if not isinstance(contract_payload, dict):
        raise SystemExit("[error] mechanics/checkpoint/parts/checkpoint-to-memory-mapping/examples/checkpoint_to_memory_contract.example.json must contain an object")

    targets = targets_payload.get("targets")
    if not isinstance(targets, list):
        raise SystemExit("[error] mechanics/writeback/parts/runtime-and-temperature/generated/runtime_writeback_targets.min.json must contain targets")

    contract_rules = contract_payload.get("mapping_rules")
    if not isinstance(contract_rules, list):
        raise SystemExit("[error] mechanics/checkpoint/parts/checkpoint-to-memory-mapping/examples/checkpoint_to_memory_contract.example.json must contain mapping_rules")
    ...
    for item in targets:
        if not isinstance(item, dict):
            raise SystemExit("[error] mechanics/writeback/parts/runtime-and-temperature/generated/runtime_writeback_targets.min.json target entries must be objects")
        runtime_surface = item.get("runtime_surface")
        if not isinstance(runtime_surface, str):
            raise SystemExit("[error] mechanics/writeback/parts/runtime-and-temperature/generated/runtime_writeback_targets.min.json target entries must include runtime_surface")

        contract_rule = rules_by_surface.get(runtime_surface)
        if contract_rule is None:
            raise SystemExit(
                f"[error] mechanics/writeback/parts/runtime-and-temperature/generated/runtime_writeback_targets.min.json references unknown runtime_surface {runtime_surface!r}"
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        ...
        if current != payload:
            raise SystemExit(
                "[error] mechanics/writeback/parts/runtime-and-temperature/generated/runtime_writeback_intake.min.json is out of date; "
                "run mechanics/writeback/parts/runtime-and-temperature/scripts/generate_runtime_writeback_intake.py"
            )
        print("[ok] mechanics/writeback/parts/runtime-and-temperature/generated/runtime_writeback_intake.min.json is current")
        return 0

    write_output(payload)
    print("[ok] wrote mechanics/writeback/parts/runtime-and-temperature/generated/runtime_writeback_intake.min.json")
    return 0
