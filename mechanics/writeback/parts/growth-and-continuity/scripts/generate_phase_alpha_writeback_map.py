...
SOURCE_PATH = PART_ROOT / "examples" / "phase_alpha_writeback_map.example.json"
OUTPUT_PATH = PART_ROOT / "generated" / "phase_alpha_writeback_map.min.json"
...
def build_phase_alpha_writeback_map_payload() -> dict[str, object]:
    ...
    if not isinstance(payload, dict):
        raise SystemExit("[error] mechanics/writeback/parts/growth-and-continuity/examples/phase_alpha_writeback_map.example.json must contain an object")
    playbooks = payload.get("playbooks")
    if not isinstance(playbooks, list):
        raise SystemExit("[error] mechanics/writeback/parts/growth-and-continuity/examples/phase_alpha_writeback_map.example.json must contain playbooks")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        ...
        if current != payload:
            raise SystemExit(
                "[error] mechanics/writeback/parts/growth-and-continuity/generated/phase_alpha_writeback_map.min.json is out of date; "
                "run mechanics/writeback/parts/growth-and-continuity/scripts/generate_phase_alpha_writeback_map.py"
            )
        print("[ok] mechanics/writeback/parts/growth-and-continuity/generated/phase_alpha_writeback_map.min.json is current")
        return 0

    write_output(payload)
    print("[ok] wrote mechanics/writeback/parts/growth-and-continuity/generated/phase_alpha_writeback_map.min.json")
    return 0
