...
OUTPUT_PATH = REPO_ROOT / "mechanics" / "writeback" / "generated" / "growth_refinery_writeback_lanes.min.json"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        ...
        if current != payload:
            raise SystemExit(
                "[error] mechanics/writeback/generated/growth_refinery_writeback_lanes.min.json is out of date; "
                "run mechanics/writeback/scripts/generate_growth_refinery_writeback_lanes.py"
            )
        print("[ok] mechanics/writeback/generated/growth_refinery_writeback_lanes.min.json is current")
        return 0

    write_output(payload)
    print("[ok] wrote mechanics/writeback/generated/growth_refinery_writeback_lanes.min.json")
    return 0
