...
INTAKE_PATH = REPO_ROOT / "mechanics" / "writeback" / "generated" / "runtime_writeback_intake.min.json"
OUTPUT_PATH = REPO_ROOT / "mechanics" / "writeback" / "generated" / "runtime_writeback_governance.min.json"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        ...
        if current != payload:
            raise SystemExit(
                "[error] mechanics/writeback/generated/runtime_writeback_governance.min.json is out of date; "
                "run mechanics/writeback/scripts/generate_runtime_writeback_governance.py"
            )
        print("[ok] mechanics/writeback/generated/runtime_writeback_governance.min.json is current")
        return 0

    write_output(payload)
    print("[ok] wrote mechanics/writeback/generated/runtime_writeback_governance.min.json")
    return 0
