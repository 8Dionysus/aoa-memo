...
SOURCE_PATH = REPO_ROOT / "mechanics" / "writeback" / "examples" / "phase_alpha_writeback_map.example.json"
OUTPUT_PATH = REPO_ROOT / "mechanics" / "writeback" / "generated" / "phase_alpha_writeback_map.min.json"
...
def build_phase_alpha_writeback_map_payload() -> dict[str, object]:
    ...
    if not isinstance(payload, dict):
        raise SystemExit("[error] mechanics/writeback/examples/phase_alpha_writeback_map.example.json must contain an object")
    playbooks = payload.get("playbooks")
    if not isinstance(playbooks, list):
        raise SystemExit("[error] mechanics/writeback/examples/phase_alpha_writeback_map.example.json must contain playbooks")
    ...
    return {
        ...
        "phase": "alpha",
        "source_of_truth": {
            "phase_alpha_writeback_map": "mechanics/writeback/examples/phase_alpha_writeback_map.example.json",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        ...
        if current != payload:
            raise SystemExit(
                "[error] mechanics/writeback/generated/phase_alpha_writeback_map.min.json is out of date; "
                "run mechanics/writeback/scripts/generate_phase_alpha_writeback_map.py"
            )
        print("[ok] mechanics/writeback/generated/phase_alpha_writeback_map.min.json is current")
        return 0

    write_output(payload)
    print("[ok] wrote mechanics/writeback/generated/phase_alpha_writeback_map.min.json")
    return 0
