...
def build_runtime_writeback_intake_payload() -> dict[str, object]:
    ...
    if not isinstance(targets, list):
        raise SystemExit("[error] mechanics/writeback/generated/runtime_writeback_targets.min.json must contain targets")
    ...
    for item in targets:
        if not isinstance(item, dict):
            raise SystemExit("[error] mechanics/writeback/generated/runtime_writeback_targets.min.json target entries must be objects")
        runtime_surface = item.get("runtime_surface")
        if not isinstance(runtime_surface, str):
            raise SystemExit("[error] mechanics/writeback/generated/runtime_writeback_targets.min.json target entries must include runtime_surface")

        contract_rule = rules_by_surface.get(runtime_surface)
        if contract_rule is None:
            raise SystemExit(
                f"[error] mechanics/writeback/generated/runtime_writeback_targets.min.json references unknown runtime_surface {runtime_surface!r}"
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        ...
        if current != payload:
            raise SystemExit(
                "[error] mechanics/writeback/generated/runtime_writeback_intake.min.json is out of date; "
                "run mechanics/writeback/scripts/generate_runtime_writeback_intake.py"
            )
        print("[ok] mechanics/writeback/generated/runtime_writeback_intake.min.json is current")
        return 0

    write_output(payload)
    print("[ok] wrote mechanics/writeback/generated/runtime_writeback_intake.min.json")
    return 0
