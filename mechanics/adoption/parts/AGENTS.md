...
`mechanics/adoption/parts/` and every active part below it.
...
The active `adoption` part rows are selected by `PARTS.md`; each row is
...
The `adoption` part route narrows the package operation; it cannot widen memo
...
Tie each `adoption` part to one row in `mechanics/adoption/PARTS.md`; keep its
...
Use the nearest `VALIDATION.md` route for the affected `adoption` part after its
