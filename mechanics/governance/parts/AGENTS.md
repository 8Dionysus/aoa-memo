...
`mechanics/governance/parts/` and every active part below it.
...
The active `governance` part rows are selected by `PARTS.md`; each row is
...
The `governance` part route narrows the package operation; it cannot widen memo
...
Tie each `governance` part to one row in `mechanics/governance/PARTS.md`; keep its
...
Use the nearest `VALIDATION.md` route for the affected `governance` part after its
