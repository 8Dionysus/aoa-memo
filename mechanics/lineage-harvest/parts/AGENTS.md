...
`mechanics/lineage-harvest/parts/` and every active part below it.
...
The active `lineage-harvest` part rows are selected by `PARTS.md`; each row is
...
The `lineage-harvest` part route narrows the package operation; it cannot widen memo
...
Tie each `lineage-harvest` part to one row in `mechanics/lineage-harvest/PARTS.md`; keep its
...
Use the nearest `VALIDATION.md` route for the affected `lineage-harvest` part after its
