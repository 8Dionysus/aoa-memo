# Retention Parts Index

Functioning Retention memo parts live here. Each part mirrors one active row in `../PARTS.md` and keeps its own contract and validation route.

## Parts

- [Cross-repo and governance retention](cross-repo-and-governance-retention/README.md) - keeps retention review owner-routed
- [Consolidation and forgetting](consolidation-and-forgetting/README.md) - keeps semantic lifecycle changes reviewed and pre-authorized mechanical maintenance exact, recoverable, and auditable
- [Office markers](office-markers/README.md) - keeps office retention markers public-safe
- [Post-release retention](post-release-retention/README.md) - keeps post-release retention visible without execution claims

## Validation

Executable part validation lives in [parts/AGENTS](AGENTS.md#validation) and the package [AGENTS](../AGENTS.md#validation).
