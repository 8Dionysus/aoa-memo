...
`mechanics/retention/parts/` and every active part below it.
...
The active `retention` part rows are selected by `PARTS.md`; each row is
...
The `retention` part route narrows the package operation; it cannot widen memo
...
Tie each `retention` part to one row in `mechanics/retention/PARTS.md`; keep its
...
Use the nearest `VALIDATION.md` route for the affected `retention` part after its
