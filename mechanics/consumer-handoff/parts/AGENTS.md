...
`mechanics/consumer-handoff/parts/` and every active part below it.
...
The active `consumer-handoff` part rows are selected by `PARTS.md`; each row is
...
The `consumer-handoff` part route narrows the package operation; it cannot widen memo
...
Tie each `consumer-handoff` part to one row in `mechanics/consumer-handoff/PARTS.md`; keep its
...
Use the nearest `VALIDATION.md` route for the affected `consumer-handoff` part after its
