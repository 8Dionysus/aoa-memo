...
`mechanics/agon/parts/` and every active part below it.
...
The active `agon` part rows are selected by `PARTS.md`; each row is
...
The `agon` part route narrows the package operation; it cannot widen memo
...
Tie each `agon` part to one row in `mechanics/agon/PARTS.md`; keep its
...
Use the nearest `VALIDATION.md` route for the affected `agon` part after its
