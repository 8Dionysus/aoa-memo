...
def test_builder_check_and_validator():
    assert subprocess.run([sys.executable, str(ROOT / 'mechanics/agon/parts/bridge-and-evidence-seams/scripts/build_agon_epistemic_memo_bridge_registry.py'), '--check'], cwd=ROOT).returncode == 0
    assert subprocess.run([sys.executable, str(ROOT / 'mechanics/agon/parts/bridge-and-evidence-seams/scripts/validate_agon_epistemic_memo_bridge.py')], cwd=ROOT).returncode == 0
