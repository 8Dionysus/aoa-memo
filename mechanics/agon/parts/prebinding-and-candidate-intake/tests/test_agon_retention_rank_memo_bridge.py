...
def test_validator_accepts_registry():
    validator = _load(ROOT / "mechanics/agon/parts/prebinding-and-candidate-intake/scripts/validate_agon_retention_rank_memo_bridge.py")
    assert validator.main() == 0


def test_schemas_constrain_entry_and_registry():
    generated = json.loads((ROOT / "mechanics/agon/parts/prebinding-and-candidate-intake/generated/agon_retention_rank_memo_bridge_registry.min.json").read_text(encoding="utf-8"))
    entry_schema = json.loads((ROOT / "mechanics/agon/parts/prebinding-and-candidate-intake/schemas/agon-retention-rank-memo-bridge.schema.json").read_text(encoding="utf-8"))
    registry_schema = json.loads((ROOT / "mechanics/agon/parts/prebinding-and-candidate-intake/schemas/agon-retention-rank-memo-bridge-registry.schema.json").read_text(encoding="utf-8"))
