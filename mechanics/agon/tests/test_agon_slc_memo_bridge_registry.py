...
OUT = ROOT / 'mechanics/agon/generated/agon_slc_memo_bridge_registry.min.json'
...
ENTRY_SCHEMA = ROOT / 'mechanics' / 'agon' / 'schemas' / 'agon-slc-memo-bridge.schema.json'
REGISTRY_SCHEMA = ROOT / 'mechanics' / 'agon' / 'schemas' / 'agon-slc-memo-bridge-registry.schema.json'
...
def test_builder_check_and_validator():
    assert subprocess.run([sys.executable, str(ROOT / 'mechanics' / 'agon' / 'scripts' / 'build_agon_slc_memo_bridge_registry.py'), '--check'], cwd=ROOT).returncode == 0
    assert subprocess.run([sys.executable, str(ROOT / 'mechanics' / 'agon' / 'scripts' / 'validate_agon_slc_memo_bridge_registry.py')], cwd=ROOT).returncode == 0


def test_builder_reports_missing_registry_id_without_keyerror(tmp_path):
    spec = importlib.util.spec_from_file_location(
        'build_agon_slc_memo_bridge_registry',
        ROOT / 'mechanics' / 'agon' / 'scripts' / 'build_agon_slc_memo_bridge_registry.py',
    )
