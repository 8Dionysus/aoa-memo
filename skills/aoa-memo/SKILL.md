---
name: aoa-memo
description: "EXISTING MEMO ARTIFACT: select aoa-memo directly; never load aoa-memo-writeback first. FIRST TOOL TURN: read only this SKILL.md, with no pwd, AGENTS, workspace/target read, search, or other command. Then use separate completed turns in this order: contract, source-return, exactly one requested mode; finish all three before target or owner reads. Use for recall, review, or requested evolution of explicit memory or a concrete candidate/export/quarantine packet/object/lifecycle/read model. Use aoa-memo-writeback only before any memo artifact exists. Do not use for raw session retrieval, proof, routing, roles, workflows, KAG substrate, runtime storage, or generic notes."
---
